...
async function loadalldata() {
  try {
      await fetchdataJasonM();
      console.log(datanetunimKlaliXM);return;
        ...
        const tkofaItem = datanetunimKlaliXM.filter(item=>item.tesua12)[0].tesua12;
...
async function loadData(type) {
  const mas="";const mozar="";
  const response = await fetch("https://klaro360.com/api/financial-data/dynamodb/query", {
  method: "POST",
...
if(type==='gemel'){
  ...
  !item.shemkupa.includes('שריעה') && !item.ochlosiyayaad.includes('עובדי סקטור מסויים') && 
  !item.ochlosiyayaad.includes('עובדי מפעל/גוף מסויים'));
  return datanetunimKlaliXM;
}
else if(type==='pensia'){
  datanetunimKlaliXP = data;
  return datanetunimKlaliXP;
}
else if(type==='bituach'){
  datanetunimKlaliXB = data;
  return datanetunimKlaliXB;
}
...
async function fetchdataJasonM() {
  await loadData('gemel');
  
}
async function fetchdataJasonB() {
  await loadData('bituach');
 ;
}
async function fetchdataJasonP() {
  await loadData('pensia');
 ;
}
